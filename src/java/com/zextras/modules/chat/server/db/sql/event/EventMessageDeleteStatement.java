...
public class EventMessageDeleteStatement implements SqlStatement
{
  private Collection<SqlParameter> mParameters = new ArrayList<SqlParameter>();

  public EventMessageDeleteStatement(final int userId, final String eventId)
  {
    mParameters.add(new SqlParameter<Integer>(1, "USERID", userId));
    mParameters.add(new SqlParameter<String>(2, "EVENTID", eventId));
  }

  @Override
  public String sql() {
    return "DELETE FROM chat.EVENTMESSAGE WHERE USERID=? AND EVENTID=?";
  }
