...
public class XmppSendSessionEstablished implements ChatOperation {

  private EventId mEventId;
  private XmppSession mSession;

  public XmppSendSessionEstablished(EventId eventId, XmppSession session) {
    mEventId = eventId;
    mSession = session;
  }
  ...
  {
    String domain = mSession.getDomain();
    Event event = new EventXmppSessionEstablished(
      mEventId,
      domain
    );
    mSession.getEventQueue().queueEvent(event);
