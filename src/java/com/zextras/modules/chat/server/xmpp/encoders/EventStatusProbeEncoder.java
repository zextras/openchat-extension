...
public class EventStatusProbeEncoder extends XmppEncoder
{
  private final EventStatusProbe mEventStatusProbe;

  public EventStatusProbeEncoder(EventStatusProbe eventStatusProbe, SchemaProvider schemaProvider) {
  ...
  public void encode(OutputStream outputStream, SpecificAddress target, boolean extensions) throws XMLStreamException
  {
    ...
    sw.writeAttribute("id", mEventStatusProbe.getId().toString());
    sw.writeAttribute("to", target.resourceAddress());
    sw.writeAttribute("from",mEventStatusProbe.getSender().resourceAddress());
    sw.writeAttribute("type", "probe");
    sw.close();
  }
}
